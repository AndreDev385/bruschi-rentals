...
- **Authentication**: Auth0 with passwordless login (email)
...
| `sendLoginCode`       | Send passwordless login email with verification code (pre-checked via `/verification/email-status`) | No            | 4/hr/email+IP |
...
| `/login`            | `login.astro`            | Login page with email-code option             |
...
The landing page contains a multi-step form (`FormWizard.tsx`) for collecting client preferences. The form persists data to localStorage so users can navigate away and return.
...
- **Price Range Fetching**: When user completes step 2 (Apartment Type), the form automatically fetches available price ranges for the selected neighborhood/type combination via `GET /api/v1/apartments/price-range`
- **Origin Tracking**: The form reads the `origin` query parameter from the URL (defaults to "Organic" if not provided)
- **Form Persistence**: Data is saved to localStorage on every step change (except `termsAccepted` which requires explicit consent each time)
- **Auto-advance**: Steps auto-advance after completion except for Date Picker (step 4)
...
4. Backend creates: Client + Lead + ClientPreferences records
5. User is redirected to `/account-created` page
